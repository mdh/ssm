...
Now mark the method as an event (in this examplet we can only transition from 
:pending state to :active state):

  event :activate_account, :pending => :active

That's it! 
You can now call the method and the state will automatically change.
If the state change is not allowed, an error is raised.
